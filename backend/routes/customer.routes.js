...
// @route   PATCH /api/customers/:id/recovery
// @desc    Toggle customer recovery status
// @access  Private (Admin/Officer)
router.patch('/:id/recovery', protect, authorize('ADMIN', 'OFFICER'), async (req, res) => {
  try {
    const customer = await Customer.findById(req.params.id);
    if (!customer) {
      return res.status(404).json({ message: 'Customer not found' });
    ...
    // Update recovery status based on request body
    const { isRecovered, recoveryDate, recoveredBy } = req.body;
    
    customer.isRecovered = isRecovered;
    
    // Set recovery information if marked as recovered
    if (isRecovered) {
      customer.recoveryDate = recoveryDate || new Date();
      customer.recoveredBy = recoveredBy || req.user.username;
    } else {
      customer.recoveryDate = null;
      customer.recoveredBy = null;
    }
